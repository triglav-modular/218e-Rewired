...
- The blend level is **fixed at 0, linear**. The machinery for blending
  toward the curve is still in the firmware — the level lives in the
  repurposed velocity-min byte `state+0x2DB` as `0xA0 | level` — but nothing
  writes it any more, and the first-use bootstrap resets it to `0xA0`. That
  reset matters: the byte is factory state rather than ours, so it survives a
  flash, and an instrument that once had a curve dialled in would otherwise
  keep it with no control left to undo it.
...
| `0x80014300` | knob 3 pressure-floor handler (knob 4 is factory) |
