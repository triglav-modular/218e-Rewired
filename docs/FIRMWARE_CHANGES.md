...
Generated by `pitch_table()` from `[pitch]`: `octave_volts` (the numbers that
used to live in the uTune) and `tracking_csv` (measured cents per key). The
table is indexed from the 208p's 0 V pitch, which is an **A**, while the bottom
key is a **C** — hence `tracking_offset_semitones = 3`.
