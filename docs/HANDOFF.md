...
- **Settings over MIDI** — stage 1's firmware is built (2026-09-22), laid
  ...
  cells, layout 2, the live bytes, the restart, the page codec) and phase
  B (the four knob roles decided at boot by dispatchers on their pool
  words, out of the image marker) are built (2026-09-23). Phases C..H
  move the other options one by one. (Dispatcher event 32 at `0x80004fc2`
