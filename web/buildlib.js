...
var BUILDLIB = (function () {
    ...
    var MARKER_EXCLUDES = { knob1: 1, knob2: 1, knob3: 1, knob4: 1, pattern_count: 1,
                            ...
                            pressure_fix: 1, pressure_portamento: 1 };
    ...
    //
    // Two different problems wearing one name.  The options are a
    // RECOMMENDATION the page makes, re-tuned per release and deliberately
    // separate from config/218e.toml - so a snapshot of them would freeze a
    // returning visitor on the defaults of whatever version they first saw,
    // and an option added default-on later would come back off from an older
    // save.  Only the DEVIATIONS are kept: a control nobody touched follows
    // the page.  The tunings, the pattern bank, the calibration and the
    // factory image are the visitor's own data and are kept whole, by the
    // page rather than here.
    //
    // Both halves are here rather than in app.js so a test can reach them;
    // the page holds the DOM half.

    function settingsDiff(now, base) {
        var out = {};
        Object.keys(now).forEach(function (k) {
            if (!base.hasOwnProperty(k) ||
                    JSON.stringify(now[k]) !== JSON.stringify(base[k])) {
                out[k] = now[k];
            }
        });
        return out;
    ...
    // The page's own defaults, overridden only where the save holds a value
    // of the same shape.  A key the page no longer offers is ignored rather
    // than restored; a key the save does not carry keeps today's default,
    // which is what lets a new option's default reach someone who saved
    // before it existed.  Nothing here is trusted: a saved blob is
    // script-writable storage and can be anything at all.
    function settingsShape(v, like) {
        if (typeof like === 'boolean') return typeof v === 'boolean';
        if (typeof like === 'number') return typeof v === 'number' && isFinite(v);
        if (typeof like === 'string') return typeof v === 'string';
        return false;
    }
    function settingsPick(stored, base) {
        var out = {};
        Object.keys(base).forEach(function (k) {
            var has = stored && typeof stored === 'object' &&
                      Object.prototype.hasOwnProperty.call(stored, k);
            out[k] = has && settingsShape(stored[k], base[k]) ? stored[k] : base[k];
        });
        return out;
    }

    // The order a restore applies things in, and the page walks exactly this
    // list - so the dependencies below are enforced by the array rather than
    // by the order somebody happened to write the handlers in.  Three of
    // these pairs are load-bearing:
    //
    //   pitch_offset before anything calibration.  Changing the offset
    ...
    //   already be there or it is overwritten by the seed.
    //
    //   pressure_fix before pressure_portamento.  Portamento needs the fix
    //   for its data and syncPortamento force-clears it when the fix is off,
    //   so restoring portamento first loses it.
    var SETTINGS_ORDER = [
        'pitch_offset', 'volts_per_octave',
        'patterns',
