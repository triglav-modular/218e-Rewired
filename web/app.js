    ...
    var KIT = {
        dlMac: {
            ...
            note: function (r, partial) { return readme(r, [
                ...
                '',
                'macOS marks everything that arrived in a download as quarantined,',
                'and this software is not signed, so it will be blocked. Clear the',
                'whole folder once, in Terminal:',
                '',
                '    xattr -dr com.apple.quarantine .',
                '',
                'run from inside the unzipped folder. Then double-click',
                '',
                '    Program218e_v3_Rewired_macOS.command',
                '',
                'The flasher clears the quarantine on dfu-programmer and its library',
                'once it is running, and will offer to. The one thing it cannot clear',
                'is its own, because macOS stops it before its first line runs, which',
                'is what the command above is for.',
                '',
                'Control-clicking and choosing Open does NOT work any more. macOS 15',
                'removed that shortcut; the dialog it shows now offers only "Move to',
                'Trash" or "Done". Without the command above, the route is System',
                'Settings > Privacy & Security > Open Anyway, once per binary.',
                '',
                'Running it from Terminal also sidesteps the block entirely:',
                '',
