...
(function () {
    ...
    (function capabilities() {
        var missing = [];
        if (!FLASH.available.midi) missing.push('Web MIDI (to request DFU)');
        if (!FLASH.available.usb) missing.push('WebUSB (to reach the bootloader)');
        var el = $('caps');
        if (!missing.length) {
            el.className = 'msg ok';
            el.textContent = 'This browser has Web MIDI and WebUSB.';
        } else {
            el.className = 'msg bad';
            el.textContent = 'This browser is missing: ' + missing.join(', ') +
                '.\nWebUSB is Chrome and Edge only — Safari and Firefox have both ' +
                'declined to implement it.';
        }
    })();

    $('enterDfu').addEventListener('click', function () {
        FLASH.enterDFU(flog).catch(function (e) { flog('✗ ' + e.message); });
    });

    $('connectDfu').addEventListener('click', function () {
        FLASH.disconnect(session).then(function () {
            return FLASH.connect(flog);
        }).then(function (s) {
            session = s;
            flog('✓ Attached to the bootloader.');
    ...
    $('preflight').addEventListener('click', function () {
        if (!session) return;
        FLASH.readState(session, flog).then(function (s) {
            if (s.state === 2 || s.state === 5) {
                flog('✓ Bootloader is idle and responding.');
            } else {
                flog('! Unexpected state — the bootloader may need a power cycle.');
            }
            if (!state.result) {
                flog('  (No image built yet — build one above before flashing.)');
            } else {
                flog('  Image ready: SHA-256 ' + state.result.sha256);
            }
            flog('  Programming is not implemented here; use the .command flasher.');
        }).catch(function (e) { flog('✗ ' + e.message); });
    });
