    ...
    protected void run() throws Exception {
        ...
        word(feature("cv_transpose")
             ? (feature("cv_jack")
                ...
                ? 0x8001eb20L : 0x8001e7c0L)
             : 0x8001a480L); // latch watch + poly-MIDI boot force + common-mode
...
// Pitch-aware latch: latched notes are pitches held in slots.  A slot k
        ...
        if (feature("cv_jack")) {
            ...
            emit("LD.SH R8,R9[0x2f0]");                           // the jack, raw
            emit(String.format("LDDPC R10,0x%x", cvPool + 8));    // the settings mirror
        ...
        word(0x00000000L); // retired: the zero, read from the mirror now
