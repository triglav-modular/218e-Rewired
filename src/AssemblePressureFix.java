...
public class AssemblePressureFix extends GhidraScript {
    ...
    // The external beat's deadline, in milliseconds after the ACCEPTED EDGE.
    // Zero builds the block but never calls it, which is the behaviour that
    // shipped: the gate goes out on the first flush after the claim and
    // carries whatever the path upstream of the claim happened to cost.
    private int deadlineMs() {
        return twoPhaseBeat() ? number("clock_deadline_ms", 4, 0, 8) : 0;
    }

    // Whether the external beat's pitch is held back to the gate's own
    // transfer.  A deadline holds the GATE at edge + D; nothing held the
    // PITCH, so a step's new note reached the DAC as much as D early and was
    // audible under the previous note's gate - the clock bleeding into the
    // pitch signal.  With a settle configured the early pitch is the whole
    // point (the CV is meant to travel before the trigger), so only the
    // settle-less external beat is held, which restores exactly the
    // electrical behaviour claim 1 always had: pitch and gate on one
    // transfer.
    private boolean holdPitchToGate() {
        return deadlineMs() > 0
    ...
    protected void run() throws Exception {
        ...
        long kbSel = 0x8001fb80L, kbK1 = kbSel + 0x20, kbRhythm = kbSel + 0x60, kbVib = kbSel + 0xa0;
        long kbEarly = kbSel + 0xc0, kbLate = kbSel + 0xe0, kbLatch1 = kbSel + 0x100, kbLatch3 = kbSel + 0x130;
        long kbPool = kbSel + 0x160, kbEnd = kbSel + 0x1a0;
        ...
        long jkEntry = 0x8001fea0L, jkOff = jkEntry + 0x1c, jkDone = jkEntry + 0x20, jkPool = jkEntry + 0x24, jkEnd = jkEntry + 0x30;
        ...
        long scStore = scEntry + 0x70, scStop = scEntry + 0x92, scOut = scEntry + 0x9c;
        long scPool = scEntry + 0xa0, scEnd = scEntry + 0xb0;
        ...
        long rsEntry = pcEnd, rsSpin = rsEntry + 0x12, rsPool = rsEntry + 0x14, rsEnd = rsEntry + 0x20;
        ...
        long opEntry = obsEnd, opUp = opEntry + 0x10, opUp2 = opEntry + 0x18, opBias = opEntry + 0x24;
        long opPool = opEntry + 0x30, opEnd = opEntry + 0x40;
        ...
        emit("LD.W R12,SP++");
        emit("MOV R8,R12");
        emit(String.format("SUB R8,-0x%x", remapOffset));
        ...
        emit("LDM SP++,R7,PC");
        padTo(0x800199f8L);
        word(0x00006840L); // per-semitone tracking-corrected curve table, in the settings mirror
        word(0x00003560L); // global state base
        ...
        emit("LSR R8,0x3");
        emit(String.format("MCALL PC[0x%x]", kbPool + 48));   // knob1_blend_latch: 0x60f2, and 0x6d38 while blending
        padTo(0x80019d6aL);
        emit("MOV R11,0x1");
        emit("MCALL PC[0x8001ddd4]");
        emit("CP.W R9,0x0");
        ...
        emit(String.format("MCALL PC[0x%x]", kbPool + 52));   // knob3_latch: 0x60ea, or zero when knob 3 is factory
        padTo(0x80019d94L);
        ...
        begin(0x8001a268L);
        word(0x8001e100L); // pulse_guard, in front of the real pulse-high routine
        emit("MOV R8,0x60ee");
        ...
        padTo(0x8001e120L);
        word(0x00003560L); // global state base
        word(0x800077f8L); // the real pulse-high routine
        finish("pulse_guard", 0x8001e128L);
        ...
        word(number("transpose_cv_filter_shift", 2, 0, 4) > 0 ? cdEntry : 0x8001e7c0L);
...
// Pitch-aware latch: latched notes are pitches held in slots.  A slot k
        ...
        emit("MOV PC,R9");
        padTo(0x8001e158L);
        word(0x80013434L); // the factory's float-to-int
        ...
        if (cvShift > 0) {
            ...
            emit(String.format("ASR R8,0x%x", cvShift));
            emit("ADD R8,R12");
            ...
            emit(String.format("LDDPC R9,0x%x", cvFilterPool + 4));
            emit("MOV PC,R9");
            padTo(cvFilterPool);
            word(0x00003560L);                                    // global state base
            word(0x8001e7c0L);                                    // cv_transpose's entry
            finish("cv_filter", 0x8001eb48L);
        }
        ...
        long cvClamp  = 0x8001e7dcL;
        long cvSlot   = 0x8001e7eaL;
        ...
        long cvRecalc = 0x8001e834L;
        long cvHaveN  = 0x8001e83eL;
        long cvCapped = 0x8001e84aL;
        long cvBuild  = 0x8001e866L;
        long cvLoop   = 0x8001e87eL;
        long cvWrap   = 0x8001e884L;
        long cvNoWrap = 0x8001e890L;
        long cvDone   = 0x8001e8aaL;
        ...
        emit(String.format("MCALL PC[0x%x]", cvPool + 12));       // jack_read -> R8
        padTo(cvClamp);
        ...
        emit(String.format("LDDPC R2,0x%x", cvPool + 8));         // the settings mirror
        ...
        emit(String.format("MCALL PC[0x%x]", cvPool + 40));       // preset_degrees_gate -> R12, zero with the option off
        emit("ADD R1,R12");
        emit("MOV R3,0x60fa");
        ...
        emit("ABS R11");
        emit("LSR R12,R2,0x1");
        emit(String.format("LDDPC R9,0x%x", cvPool + 8));         // the settings mirror
        ...
        emit(String.format("MCALL PC[0x%x]", cvPool + 32));       // stamps: against the keys, not the slots
        emit(String.format("LDDPC R2,0x%x", cvPool + 28));        // the period cell's address
        ...
        emit(String.format("MCALL PC[0x%x]", cvPool + 36));       // stamps back against the slots; the sounding base
        padTo(cvDone);
        emit("LDM SP++,R0,R1,R2,R3,R4,R7,PC");
        padTo(cvPool);
        word(0x8001a480L); // per-scan housekeeping: latch watch + poly-MIDI boot force + common-mode
        word(0x00003560L); // global state base
        word(0x00006800L); // the settings mirror: the period, its zero and the hysteresis are cells 5, 6 and 7
        word(jkEntry);     // jack_read: the reading less its zero, clamped, or zero by the live byte
        word(0x00000000L); // retired: the hysteresis, read from the mirror now
        word(0x000069a0L); // the keys-per-period table, in the settings mirror
        word(0x000068e0L); // the three tuning tables, in the settings mirror
        word(0x00006814L); // the period: number cell 10 of the settings mirror
        word(cvStampsPre);
        word(cvStampsPost);
        ...
        long lppEntry = 0x8001eca8L, lppLoop = 0x8001ecdcL, lppOwn = 0x8001ed00L;
        long lppHave  = 0x8001ed08L, lppNext = 0x8001ed38L, lppSave = 0x8001ed40L;
        long lppDone  = 0x8001ed48L, lppPool = 0x8001ed4cL;
        begin(lppEntry);
        ...
        emit("CP.W R3,0x0");
        emit(String.format("BR{ne} 0x%x", lppDone));
        emit("MOV R3,0x60f3");
        emit("LD.UB R3,R3[0x0]");                                 // the count now
        emit("MOV R2,0x6092");
        emit("LD.UB R2,R2[0x0]");                                 // and as the last rebuild saw it
        emit("CP.W R3,R2");
        emit(String.format("BR{eq} 0x%x", lppDone));              // the preset did not move
        emit("MOV R0,0x62e2");
        ...
        emit("CP.W R8,0x0");
        emit(String.format("BR{eq} 0x%x", lppSave));
        emit("LD.UB R0,R0[0x0]");
        emit("CP.W R0,0x0");
        emit(String.format("BR{ne} 0x%x", lppSave));              // transpose: the set follows
        emit("MOV R1,0x1c");
        padTo(lppLoop);
        emit("MOV R0,0x377b");                                    // state+0x21b, the held flags
        emit("LD.UB R0,R0[R1 << 0x0]");
        emit("CP.W R0,0x1");
        emit(String.format("BR{ne} 0x%x", lppNext));
        emit("MOV R0,0x6504");                                    // the owner map, key plus one
        emit("LD.UB R0,R0[R1 << 0x0]");
        emit("CP.W R0,0x0");
        emit(String.format("BR{eq} 0x%x", lppOwn));
        emit("SUB R0,0x1");
        emit(String.format("RJMP 0x%x", lppHave));
        padTo(lppOwn);
        ...
        emit("MOV R9,0x6092");
        emit(String.format("MCALL PC[0x%x]", lppPool));           // as it stood before the preset moved
        ...
        padTo(lppNext);
        emit("SUB R1,0x1");
        emit(String.format("BR{ge} 0x%x", lppLoop));
        padTo(lppSave);
        emit("MOV R2,0x6092");
        emit("ST.B R2[0x0],R3");
        padTo(lppDone);
        ...
        padTo(lppPool);
        word(0x8001e420L); // preset_entry
        finish("latch_preset_pin", 0x8001ed54L);
        ...
        long cvStampsDone     = 0x8001ec9cL;
        long cvStampsPool     = 0x8001eca0L;
        begin(cvStampsPre);
        emit("STM --SP,R0,R1,R2,R7,R8,R9,R10,LR");
        emit("MOV R7,SP");
        emit(String.format("LDDPC R0,0x%x", cvStampsPool));       // global state base
        emit("MOV R1,0x1c");
        padTo(cvStampsPreLoop);
        emit("ADD R8,R0,R1 << 0x0");
        emit("LD.UB R8,R8[0x21b]");                               // held?
        emit("CP.W R8,0x1");
        emit(String.format("BR{ne} 0x%x", cvStampsPreNext));
        emit("MOV R8,0x6504");
        emit("LD.UB R2,R8[R1 << 0x0]");                           // owner, plus one
        emit("CP.W R2,0x0");
        emit(String.format("BR{eq} 0x%x", cvStampsPreNext));
        emit("SUB R2,0x1");
        emit("CP.W R2,R1");
        emit(String.format("BR{eq} 0x%x", cvStampsPreNext));      // its own key's slot
        emit("MOV R8,0x854");
        ...
        emit("CP.W R2,0x0");
        emit(String.format("BR{eq} 0x%x", cvStampsOurs));         // nothing published yet
        ...
        emit("STM --SP,R0,R1,R2,R7,R8,R9,R10,LR");
        emit("MOV R7,SP");
        emit(String.format("LDDPC R0,0x%x", cvStampsPool));
        emit("MOV R1,0x1c");
        padTo(cvStampsPostLoop);
        emit("ADD R8,R0,R1 << 0x0");
        emit("LD.UB R8,R8[0x21b]");
        emit("CP.W R8,0x1");
        emit(String.format("BR{ne} 0x%x", cvStampsPostNext));
        emit("MOV R8,0x6504");
        emit("LD.UB R2,R8[R1 << 0x0]");
        emit("CP.W R2,0x0");
        emit(String.format("BR{eq} 0x%x", cvStampsPostNext));
        emit("SUB R2,0x1");
        emit("CP.W R2,R1");
        emit(String.format("BR{eq} 0x%x", cvStampsPostNext));
        emit("MOV R8,0x854");
        ...
        emit(String.format("MCALL PC[0x%x]", cvStampsPool + 4));
        ...
        padTo(cvStampsPool);
        word(0x00003560L); // global state base
        word(0x8001eca8L); // latch_preset_pin
        finish("cv_stamps", 0x8001eca8L);
        ...
        emit(String.format("RJMP 0x%x", mtCommon));
        padTo(mtHeld);
        ...
        word(0x00003560L); // global state base
        word(0x8001e488L); // the sequencer step's own preset degrees
        ...
        long srcEntry  = 0x8001e320L;
        long srcSlot   = 0x8001e33cL;
        ...
        long scsDone  = 0x8001e418L;
        long scsPool  = 0x8001e41cL;
        begin(scsEntry);
        ...
        emit("MOV R12,R10");
        emit(String.format("MCALL PC[0x%x]", scsPool));           // slot[key + e]
        emit("MOV R11,R12");
        emit("MOV R12,0x6091");
        emit("MOV R8,0x62fe");
        emit("LD.UB R8,R8[0x0]");
        ...
        emit("MOV R12,R8");
        emit(String.format("MCALL PC[0x%x]", scsPool));           // slot[key + N - X + e]
        emit("SUB R12,R11");
        emit("LD.W R8,SP++");
        emit("ADD R8,R12");
        padTo(scsDone);
        ...
        long peEntry = 0x8001e420L, peSlot = 0x8001e43cL, peWrap = 0x8001e454L;
        long peDown = 0x8001e460L, peNoWrap = 0x8001e46aL, pePool = 0x8001e478L;
        begin(peEntry);
        ...
        padTo(msdDone);
        emit("MOV PC,LR");
        finish("midi_step_degree", 0x8001e4b8L);
        ...
        word(0x00003560L); // global state base
        word(0x8001b550L); // the take's reference, adopted on the first step
        word(0x00006800L); // the settings mirror
        finish("seq_strip", 0x8001b610L);
        ...
        if (block("clock_latency")) {
            ...
            emit("LDM SP++,R7,PC");
            padTo(0x8001bca0L);
            word(0x8001e100L);          // pulse_guard -> the real pulse-high routine
        ...
        if (deadlineMs() > 0) {
            ...
            emit("LDM SP++,R7,PC");
            padTo(0x8001b9c0L);
            word(block("clock_thresholds") ? 0x8001c3c0L : 0x8001c400L); // clock_thresholds, then clock_service
            word(feature("scan_profiler") ? 0x8001a540L : 0x80004c64L);
            word(0x8001c100L); // pending-output service: the fast trigger
            finish("clock_scan", 0x8001b9ccL);
        } else {
            emit("STM --SP,R7,LR");
            emit("MOV R7,SP");
        ...
        word(0x8001d280L);
        finish("persist_boot", 0x8001d600L);
        ...
        if (block("clock_fast_trigger")) {
            // Nor an old flush CLAIM.  Without a deadline only a beat with a
            // settle configured was ever left holding one; with one every
            // external beat is, so a claim surviving the transition would put
            // a trigger out after the transport had stopped.
            emit("MOV R10,0x625b");
            emit("ST.B R10[0x0],R8");
        }
        ...
        padTo(0x8001e5d0L);
        emit("LDM SP++,R7,PC");
        finish("latch_state_toggle", 0x8001e600L);
        ...
        if (feature("arp_latch")) {
            ...
            emit(String.format("MCALL PC[0x%x]", ltPool + 20));
        ...
        padTo(0x8001e720L);
        word(0x00003560L);              // global state base
        word(0x8001e580L);              // latch_state_toggle
        word(0x8000698cL);              // select_pad(0..3)
        word(0x80006808L);              // led_set(ch)
        word(0x800068ccL);              // led_clear(ch)
        word(0x8000673cL);              // led_flush()
        finish("latch_state", 0x8001e740L);
        ...
        finish("clock_thresholds", 0x8001c400L);
        ...
        word(block("clock_latency") ? 0x8001bbc0L : 0x8001e100L); // via pulse_guard
        word(0x8001c600L);
        ...
        // expressed in scans reaches it least of all.  A wait counted from
        ...
        begin(0x8001bd40L);
        int dSettleExt = settleMsFor(number("clock_settle_scans", 0, 0, 3));
        int dSettleInt = settleMsFor(number("gate_settle_scans", 1, 0, 3));
        emit("STM --SP,R7,LR");
        emit("MOV R7,SP");
        emit("CP.W R12,0x0");
        ...
        if (dSettleExt > 0) {
            ...
            emit(String.format("MOV R9,0x%x", dSettleExt));
            emit("MUL R9,R10,R9");
            emit("MFSR R11,COUNT");
        ...
        emit("LSR R9,0x1");
        emit(String.format("MOV R12,0x%x", deadlineMs() + dSettleExt));
        emit("CP.W R12,R9");
        ...
        if (dSettleInt > 0) {
            ...
            emit(String.format("MOV R9,0x%x", dSettleInt));
            emit("MUL R9,R10,R9");
            emit("MFSR R11,COUNT");
            emit("ADD R11,R9");
        } else {
        ...
        padTo(0x8001be28L);
        word(0x8000ba9cL); // the factory DAC transfer the pulse paths use
        finish("clock_deadline", 0x8001be2cL);
        ...
        emit("LDM SP++,R7,PC");
        padTo(0x8001be48L);
        word(0x80003590L); // the factory pitch-target preparation
        word(0x00003560L);
        finish("clock_pitch_target", 0x8001be50L);
        ...
        emit(String.format("MCALL PC[0x%x]", fastPool));
        ...
        if (twoPhaseBeat()) {
            ...
            if (deadline) {
                ...
                emit(String.format("BR{ne} 0x%x", fastGate));
                emit("MOV R12,0x1");
                emit(String.format("MCALL PC[0x%x]", fastPool + 12));
                emit("CP.W R12,0x0");
                emit(String.format("BR{ne} 0x%x", fastExit));
        ...
        padTo(fastPool);
        word(0x8001be30L); // prepare the selected step's transposed pitch target
        word(0x8001c0e0L); // remap, minus the per-scan chain
        word(block("clock_latency") ? 0x8001bbc0L : 0x8001e100L); // via pulse_guard
        if (deadline) word(0x8001bd40L); // the deadline, sized from the edge
        // Four pool words take the block right up to clock_capture, which
        // begins at 0x8001c200.  There is no room for a fifth.
        finish("clock_fast_trigger",
               twoPhaseBeat() ? (deadline ? 0x8001c200L : 0x8001c1f0L) : 0x8001c1e0L);
        ...
        // The flush CLAIM is cleared by seq_transport at 0x8001d688, beside
        // where that routine drops the FIFO and the in-flight flag, and not
        // here: this cave is packed to the byte, and this was never the full
        // discard path anyway - it leaves 0x6237 and the queue alone too.
        ...
        emit("LDM SP++,R7,PC");
        padTo(0x8001ba2cL);
        word(0x8001e4b8L);              // seq_record_degree -> seq_record_pitch
        ...
        finish("seq_record_degree", 0x8001e4e0L);
        ...
        word(0x80007e44L); // MIDI note off, port one
        word(0x800081f0L); // MIDI note off, port two
        word(feature("cv_transpose") ? mtPress : 0x800057a8L); // key -> MIDI note, shifted with the jack transposing
        ...
        if (twoPhaseBeat()) {
            emit("MOV R10,0x625b");
            // With a deadline the external beat is ALWAYS held: the wait is
            // not known here, it is sized at phase A from this edge's own
            // stamp, so the claim has to be the one that holds the gate even
            // when no settle was configured.
            emit(String.format("MOV R11,0x%x",
                 deadlineMs() > 0 ? 2
        ...
        begin(0x80004e72L);
        emit(String.format("MCALL PC[0x%x]", cePool + 4));   // clock_event_dispatch
        emit("NOP");
        emit("NOP");
        ...
        begin(0x800072eeL);
        emit(String.format("MCALL PC[0x%x]", ciPool + 8));   // clock_irq_dispatch
        emit("RJMP 0x80007322");
        finish("clock_irq_hook", 0x800072f4L);
        {
        ...
        emit("LDM SP++,R0,R1,R2,R3,R7,PC");
        padTo(0x8001f1b0L);
        word(0x8003d000L); // the first slot; the second is 0x800 above it
        ...
        emit("CP.W R12,0x0");
        emit(String.format("BR{ne} 0x%x", scStop));
        emit("SUB R0,0x1");
        emit(String.format("BR{ne} 0x%x", scLoop));
        emit(String.format("RJMP 0x%x", scOut));
        padTo(scStop);
        emit("MOV R8,0x6a68");
        emit("MOV R10,0x4000");
        ...
        begin(rsEntry);
        emit("MOV R8,0x0");
        emit(String.format("LDDPC R9,0x%x", rsPool));
        ...
        padTo(rsPool);
        word(0x4953504bL); // "ISPK", the ISP RAM key
        ...
        Emitter knobCaves = () -> {
        // The arp's key selector, for the factory's pool word and the
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x3");
        emit(String.format("BR{ne} 0x%x", kbK1));
        emit(String.format("LDDPC R8,0x%x", kbPool));       // the pattern gate
        emit("MOV PC,R8");
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x1");
        emit(String.format("BR{eq} 0x%x", kbK1 + 0x14));
        emit(String.format("BR{gt} 0x%x", kbK1 + 0x18));
        emit(String.format("LDDPC R8,0x%x", kbPool + 4));   // the blend
        emit("MOV PC,R8");
        padTo(kbK1 + 0x14);
        emit(String.format("LDDPC R8,0x%x", kbPool + 8));   // the six zones
        emit("MOV PC,R8");
        padTo(kbK1 + 0x18);
        emit(String.format("LDDPC R8,0x%x", kbPool + 12));  // the factory selector
        emit("MOV PC,R8");
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x1");
        emit(String.format("BR{eq} 0x%x", kbRhythm + 0x24));   // quantized
        emit(String.format("BR{lt} 0x%x", kbRhythm + 0x1c));   // spacing: the randomiser
        emit("CP.W R8,0x2");
        emit(String.format("BR{eq} 0x%x", kbRhythm + 0x2c));   // swing
        emit(String.format("LDDPC R8,0x%x", kbPool + 16));  // global state base
        emit("ST.H R8[0x38e],R12");
        emit("MOV R9,R12");
        emit("MOV PC,LR");
        padTo(kbRhythm + 0x1c);
        emit(String.format("LDDPC R8,0x%x", kbPool + 20));  // the randomiser
        emit("MOV PC,R8");
        padTo(kbRhythm + 0x24);
        emit(String.format("LDDPC R8,0x%x", kbPool + 24));  // quantized
        emit("MOV PC,R8");
        padTo(kbRhythm + 0x2c);
        emit(String.format("LDDPC R8,0x%x", kbPool + 28));  // swing
        emit("MOV PC,R8");
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x0");
        emit(String.format("BR{ne} 0x%x", kbVib + 0x10));
        emit(String.format("LDDPC R8,0x%x", kbPool + 32));  // the vibrato engine
        emit("MOV PC,R8");
        padTo(kbVib + 0x10);
        emit("MOV PC,LR");
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x1");
        emit(String.format("BR{ne} 0x%x", kbEarly + 0x10));
        emit(String.format("LDDPC R8,0x%x", kbPool + 36));  // knob4_early
        emit("MOV PC,R8");
        padTo(kbEarly + 0x10);
        emit(String.format("LDDPC R8,0x%x", kbPool + 40));  // the factory ADC event handler
        emit("MOV PC,R8");
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x1");
        emit(String.format("BR{ne} 0x%x", kbLate + 0x10));
        emit(String.format("LDDPC R8,0x%x", kbPool + 44));  // knob 4 as an octave switch
        emit("MOV PC,R8");
        padTo(kbLate + 0x10);
        emit("MOV PC,LR");
        ...
        emit("LD.UB R11,R11[0x0]");
        emit("CP.W R11,0x0");
        emit(String.format("BR{ne} 0x%x", kbLatch1 + 0x18));
        emit("MOV R11,0x6d38");
        emit("ST.B R11[0x0],R8");
        emit("MOV PC,LR");
        padTo(kbLatch1 + 0x18);
        ...
        emit("LD.UB R11,R11[0x0]");
        emit("CP.W R11,0x0");
        emit(String.format("BR{eq} 0x%x", kbLatch3 + 0xc));
        emit("MOV R8,0x0");
        padTo(kbLatch3 + 0xc);
        emit("MOV R11,0x60ea");
        emit("ST.H R11[0x0],R8");
        ...
        begin(kbPool);
        word(0x8001b050L); // the pattern gate
        word(0x8001a0a0L); // knob 1's blend selector
        word(0x8001aec0L); // knob 1's six zones
        word(0x800029a8L); // the factory selector
        word(0x00003560L); // global state base
        word(0x80019df8L); // knob 2's randomiser
        word(0x8001ea00L); // quantized
        word(0x8001b100L); // swing
        word(0x8001a350L); // the vibrato engine
        word(0x8001d920L); // knob4_early
        word(0x80004a00L); // the factory ADC event handler
        word(0x8001b010L); // knob 4 as an octave switch, the late pass
        ...
        Emitter latchCaves = () -> {
        // Note-on: latch_owner and the pitch-aware toggle while the latch is
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x0");
        emit(String.format("BR{eq} 0x%x", ltOn + 0xe));
        emit(String.format("LDDPC R8,0x%x", ltPool));       // latch_owner
        emit("MOV PC,R8");
        padTo(ltOn + 0xe);
        emit("CP.W R12,0x1c");
        emit(String.format("BR{hi} 0x%x", ltOn + 0x28));
        emit("MOV R9,0x6521");
        emit("MOV R8,R12");
        emit("SUB R8,-0x1");
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x0");
        emit(String.format("BR{eq} 0x%x", ltOff + 0xe));
        emit(String.format("LDDPC R8,0x%x", ltPool + 4));   // the latch's note-off wrapper
        emit("MOV PC,R8");
        padTo(ltOff + 0xe);
        emit("CP.W R12,0x1d");
        emit(String.format("BR{ge} 0x%x", ltOff + 0x1c));
        emit("MOV R9,0x6521");
        emit("MOV R8,0x0");
        emit("ST.B R9[R12 << 0x0],R8");   // current[key] = none
        padTo(ltOff + 0x1c);
        emit(String.format("LDDPC R8,0x%x", ltPool + 8));   // the factory note-off
        emit("MOV PC,R8");
        ...
        emit("LD.UB R11,R11[0x0]");
        emit("CP.W R11,0x0");
        emit(String.format("BR{eq} 0x%x", ltHold + 0x10));
        emit(String.format("LDDPC R10,0x%x", ltPool + 12));  // latch_hold
        emit("MOV PC,R10");
        padTo(ltHold + 0x10);
        emit("MOV PC,LR");
        ...
        emit("LD.UB R9,R9[0x0]");
        emit("CP.W R9,0x0");
        emit(String.format("BR{eq} 0x%x", ltPad + 0x10));
        emit("LD.UB R9,R8[0x1]");
        emit("CP.W R9,0x2");
        emit("MOV PC,LR");
        padTo(ltPad + 0x10);
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x0");
        emit(String.format("BR{ne} 0x%x", ltShim + 0x14));
        emit(String.format("MCALL PC[0x%x]", ltPool + 16));  // the factory latch timer
        padTo(ltShim + 0x14);
        emit("LDM SP++,R7,PC");
        finish("latch_pad_shim", ltPool);
        begin(ltPool);
        word(0x8001dde0L); // latch_owner
        word(0x8001a280L); // the latch's note-off wrapper
        word(0x80005a50L); // the factory note-off
        word(0x8001e520L); // latch_hold
        word(0x800045e8L); // the factory pads 2 & 3 latch timer
        ...
        emit("SUB R9,-0x1");
        emit("SUB R10,0x1");
        emit(String.format("BR{ne} 0x%x", obLoop));
        emit("MOV R8,0x6d38");
        emit("MOV R9,0x0");
        ...
        emit("SUB R10,0x1");
        emit(String.format("BR{ge} 0x%x", obStamps));
        emit("MOV R8,0x609e");
        ...
        emit("SUB R9,-0x2");
        emit("SUB R10,0x1");
        emit(String.format("BR{ne} 0x%x", sqcLoop1));
        ...
        emit("SUB R9,-0x1");
        emit("SUB R10,0x1");
        emit(String.format("BR{ne} 0x%x", sqcLoop3));
        ...
        Emitter jackCaves = () -> {
        // jack_read: cv_transpose's reading of the jack.  R8 = the raw
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x0");
        emit(String.format("BR{eq} 0x%x", jkOff));
        emit(String.format("LDDPC R9,0x%x", jkPool));         // global state base
        emit("LD.SH R8,R9[0x2f0]");                           // the jack, raw
        emit(String.format("LDDPC R10,0x%x", jkPool + 4));    // the settings mirror
        emit("LD.UH R10,R10[0xc]");                           // its zero, settings cell 6
        emit("SUB R8,R10");
        emit("CP.W R8,0x0");
        emit(String.format("BR{ge} 0x%x", jkDone));
        padTo(jkOff);
        emit("MOV R8,0x0");
        padTo(jkDone);
        emit("MOV PC,LR");
        ...
        emit("LD.UB R12,R12[0x0]");
        emit("CP.W R12,0x0");
        emit(String.format("BR{eq} 0x%x", pgOff));
        emit(String.format("LDDPC R9,0x%x", pgPool));
        emit("MOV PC,R9");
        padTo(pgOff);
        emit("MOV R9,0x60f3");
        ...
        emit("LD.UB R9,R9[0x0]");
        emit("CP.W R9,0x0");
        emit(String.format("BR{eq} 0x%x", gaLoad));
        emit("MOV R8,0x28");
        emit("MOV PC,LR");
        padTo(gaLoad);
        ...
        if (cvShift > 0) {
            ...
            emit("LD.UB R8,R8[0x0]");
            emit("CP.W R8,0x0");
            emit(String.format("BR{eq} 0x%x", cdPlain));
            emit(String.format("LDDPC R8,0x%x", cdPool));
            emit("MOV PC,R8");
            padTo(cdPlain);
            emit(String.format("LDDPC R8,0x%x", cdPool + 4));
            emit("MOV PC,R8");
            padTo(cdPool);
            word(cvFilter);
            word(cvEntry);
        ...
        Emitter clockCaves = () -> {
        // clock_irq_dispatch: inside the factory ISR's frame, from the hook
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x0");
        emit(String.format("BR{eq} 0x%x", ciOff));
        emit(String.format("LDDPC R8,0x%x", ciPool));
        emit("MOV PC,R8");
        padTo(ciOff);
        emit("MOV R12,0x5");
        emit("MCALL PC[0x8000732c]");   // the factory's flag getter, through the factory's own word
        emit("MOV R8,R12");
        emit(String.format("LDDPC R9,0x%x", ciPool + 4));
        emit("MOV PC,R9");
        padTo(ciPool);
        word(0x8001c200L); // the capture cave
        word(0x800072f4L); // the factory body, after the displaced pair
        word(ciEntry);
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x0");
        emit(String.format("BR{eq} 0x%x", ceOff));
        emit(String.format("LDDPC R8,0x%x", cePool));
        emit("MOV PC,R8");
        padTo(ceOff);
        emit("MOV R12,0xffff");
        emit(String.format("LDDPC R8,0x%x", cePool + 8));
        emit("MOV PC,R8");
        padTo(cePool);
        word(0x800051b0L); // past the step
        word(ceEntry);
        word(0x8000210cL); // the factory's arp step
        ...
        emit("LD.UB R8,R8[0x0]");
        emit("CP.W R8,0x0");
        emit(String.format("BR{eq} 0x%x", puOff));
        emit(String.format("LDDPC R8,0x%x", puPool));
        emit("MOV PC,R8");
        padTo(puOff);
        emit(String.format("LDDPC R8,0x%x", puPool + 4));
        emit("MOV PC,R8");
        padTo(puPool);
        word(0x8001c700L); // the clock's pulse: settle, then the flush
        word(0x8001a26cL); // pulse_defer_set
        ...
        begin(0x8000491eL);
        emit(String.format("MCALL PC[0x%x]", ltPool + 24));   // latch_pad_shim
        finish("factory_pad_latch_off", 0x80004922L);
        ...
        wordPatch("preset_quantize_pool", 0x80003914L, 0x8001e140L,
        ...
        begin(0x80003776L);
        emit(String.format("MCALL PC[0x%x]", opPool));       // -period: the panel's octave down
        finish("octave_step_down", 0x8000377aL);
        begin(0x80003788L);
        emit(String.format("MCALL PC[0x%x]", opPool + 4));   // +period: octave up
        finish("octave_step_up", 0x8000378cL);
        begin(0x80003792L);
        ...
        begin(0x800035faL);
        emit(String.format("MCALL PC[0x%x]", opPool + 12));  // its two-octave bias
        finish("octave_scale_bias", 0x800035feL);
        ...
        begin(0x800035c0L);
        emit(String.format("MCALL PC[0x%x]", opPool));       // -period: the keyboard an octave down under add-to-pitch
        finish("octave_pitch_add", 0x800035c4L);
        ...
        wordPatch("key_note_pool", 0x80005ebcL,
            block("seq_pitch") ? 0x8001bf00L : (feature("cv_transpose") ? mtPress : 0x800057a8L),
        ...
        wordPatch("midi_transpose_arp_pool", 0x80002428L, mtLive,
            "key -> MIDI note at the arp step -> shifted live");
        wordPatch("midi_transpose_poly_pool", 0x80005784L, mtLive,
            "key -> MIDI note in the polyphonic sender -> shifted live");
        wordPatch("midi_transpose_lift_pool", 0x80006284L, mtHeld,
            "key -> MIDI note at the lift -> shifted as at the press");
        wordPatch("midi_transpose_compare_pool", 0x80009830L, mtHeld,
